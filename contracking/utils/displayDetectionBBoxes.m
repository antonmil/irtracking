...
for t=1:F
    detections(t).yi=588-detections(t).yi;
    detections(t).by=546-detections(t).by;
end
...
%     text(20,100,sceneInfo.sequence,'FontSize',16);
    ...
    for k=1:length(detections(t).xp)
        ...
        plot(detections(t).xp(k),detections(t).yp(k),'.','color',detcol,'MarkerSize',dotSize*max(0.1,detections(t).sc(k)));
    end
...
%                 pause
                ...
                plot(detections(tracet).xi(k),detections(tracet).yi(k), ...
                    '.','color',detcol,'MarkerSize',dotSize*max(0.1,detections(tracet).sc(k)));
...
%     saveas(gcf,sprintf('tmp/dets/dets-%s-%04d.jpg',sceneInfo.sequence,t));
%     gd=getframe(gcf);
%     imwrite(gd.cdata,sprintf('tmp/dets-%s-%04d.jpg',sceneInfo.sequence,t));
    pause(framePause)
end
