...
global detcutoff %%% TEMPORARY
detcutoff=opt.detThreshold;
nDets=0;
...
if exist(matfile,'file')
    ...
    if scenario==41
        detections=removeWeakOnes(detections,detcutoff);
    elseif scenario>190
%         detections=removeWeakOnes(detections,-.9);
...
% [detections(:).sc]  
    if scenario>190
    detections=sigmoidify(detections,opt);
    end
